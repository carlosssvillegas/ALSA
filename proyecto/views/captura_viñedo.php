<!DOCTYPE html>
<html>
  <head>
    <meta charset="UTF-8">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    <!-- Compiled and minified CSS -->
    <link rel="stylesheet" href="../css/materialize.min.css">
    <!-- Compiled and minified JavaScript -->
    <script src="../js/materialize.min.js"></script>      
    <!--Import Google Icon Font-->
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <!--Let browser know website is optimized for mobile-->
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <!--JavaScript at end of body for optimized loading-->
    <script src="../js/javascript.js"></script>
    <link rel="stylesheet" href="../css/stylesheet.css">
    
  </head>
  <body> 
    <!-- Captura Calidad-->
    <ul id="tabs-swipe-demo" class="tabs  center">
      <li class="tab col s3"><a class="active" href="#test-swipe-1">Lunes</a></li>
      <li class="tab col s3"><a href="#test-swipe-2">Martes</a></li>
      <li class="tab col s3"><a href="#test-swipe-3">Miercoles</a></li>
      <li class="tab col s3"><a href="#test-swipe-4">Jueves</a></li>
      <li class="tab col s3"><a href="#test-swipe-5">Viernes</a></li>
      <li class="tab col s3"><a href="#test-swipe-6">Sabado</a></li>
      <li class="tab col s3"><a href="#test-swipe-7">Domingo</a></li>
    </ul>
    <!-- Captura Lunes-->
    <div id="test-swipe-1" class="col s12">
      <div class="container">
        <h4 class="center">Lunes</h4>
        <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
            <div class="row">
              <form class="col s6">
                <div class="row">
                  <div class="input-field col s12">
                    <i class="material-icons prefix">access_time</i>
                    <input type="text" class="timepicker">
                    <label for="icon_prefix4">Salida</label>
                  </div>
                </div>
              </form>
            </div>
            </div>
            <div class="row">
              <form class="col s6">
                <div class="input-field col s12">
                  <select>
                    <option value="1">Asistencia</option>
                    <option value="2">Inasistencia</option>
                    <option value="3">Permiso</option>
                    <option value="4">Vacaciones</option>
                    <option value="5">Festivo</option>
                  </select>
                    <label>Asistencia</label>
                </div>
              </form>
              <div class="row">
                <form class="col s6">
                <div class="row">
                    <div class="input-field col s12">
                      <i class="material-icons prefix">access_time</i>
                      <input type="Number">
                      <label for="icon_prefix3">Horas Extra</label>
                    </div>
                  </div>
                </form>
              </div>  
            </div>
          </div> 
          <div class="row">
            <form class="col s12">
              <div class="center">
                <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
                <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
              </div>
            </form>
          </div>
        </div>
    </div>
    <!-- Captura Martes-->
    <div id="test-swipe-2" class="col s12">
      <div class="container">
          <h4 class="center">Martes</h4>
          <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="text" class="timepicker">
                  <label for="icon_prefix4">Salida</label>
                </div>
              </div>
            </form>
          </div>
          </div>
          <div class="row">
            <form class="col s6">
              <div class="input-field col s12">
                <select>
                  <option value="1">Asistencia</option>
                  <option value="2">Inasistencia</option>
                  <option value="3">Permiso</option>
                  <option value="4">Vacaciones</option>
                  <option value="5">Festivo</option>
                </select>
                  <label>Asistencia</label>
              </div>
            </form>
            <div class="row">
              <form class="col s6">
                <div class="row">
                  <div class="input-field col s12">
                    <i class="material-icons prefix">access_time</i>
                    <input type="Number">
                    <label for="icon_prefix3">Horas Extra</label>
                  </div>
                </div>
              </form>
            </div>  
          </div>
          </div> 
          <div class="row">
            <form class="col s12">
              <div class="center">
                <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
                <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
              </div>
            </form>
          </div>
      </div>
    </div>
    <!-- Captura Miercoles-->
    <div id="test-swipe-3" class="col s12">
      <div class="container">
          <h4 class="center">Miercoles</h4>
          <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="text" class="timepicker">
                  <label for="icon_prefix4">Salida</label>
                </div>
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <form class="col s6">
            <div class="input-field col s12">
              <select>
                <option value="1">Asistencia</option>
                <option value="2">Inasistencia</option>
                <option value="3">Permiso</option>
                <option value="4">Vacaciones</option>
                <option value="5">Festivo</option>
              </select>
                <label>Asistencia</label>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="Number">
                  <label for="icon_prefix3">Horas Extra</label>
                </div>
              </div>
            </form>
          </div>  
        </div>
        </div> 
        <div class="row">
          <form class="col s12">
            <div class="center">
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
            </div>
          </form>
        </div>
      </div>
    </div>
    <!-- Captura Jueves-->
    <div id="test-swipe-4" class="col s12">
      <div class="container">
          <h4 class="center">Jueves</h4>
          <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="text" class="timepicker">
                  <label for="icon_prefix4">Salida</label>
                </div>
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <form class="col s6">
            <div class="input-field col s12">
              <select>
                <option value="1">Asistencia</option>
                <option value="2">Inasistencia</option>
                <option value="3">Permiso</option>
                <option value="4">Vacaciones</option>
                <option value="5">Festivo</option>
              </select>
                <label>Asistencia</label>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="Number">
                  <label for="icon_prefix3">Horas Extra</label>
                </div>
              </div>
            </form>
          </div>  
        </div>
        </div> 
        <div class="row">
          <form class="col s12">
            <div class="center">
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
            </div>
          </form>
        </div>
      </div>
    </div>
    <!-- Captura Viernes-->
    <div id="test-swipe-5" class="col s12">
      <div class="container">
          <h4 class="center">Viernes</h4>
          <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="text" class="timepicker">
                  <label for="icon_prefix4">Salida</label>
                </div>
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <form class="col s6">
            <div class="input-field col s12">
              <select>
                <option value="1">Asistencia</option>
                <option value="2">Inasistencia</option>
                <option value="3">Permiso</option>
                <option value="4">Vacaciones</option>
                <option value="5">Festivo</option>
              </select>
                <label>Asistencia</label>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="Number">
                  <label for="icon_prefix3">Horas Extra</label>
                </div>
              </div>
            </form>
          </div>  
        </div>
        </div> 
        <div class="row">
          <form class="col s12">
            <div class="center">
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
            </div>
          </form>
        </div>
      </div>
    </div>
    <!-- Captura Sabado-->
    <div id="test-swipe-6" class="col s12">
      <div class="container">
          <h4 class="center">Sabado</h4>
          <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="text" class="timepicker">
                  <label for="icon_prefix4">Salida</label>
                </div>
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <form class="col s6">
            <div class="input-field col s12">
              <select>
                <option value="1">Asistencia</option>
                <option value="2">Inasistencia</option>
                <option value="3">Permiso</option>
                <option value="4">Vacaciones</option>
                <option value="5">Festivo</option>
              </select>
                <label>Asistencia</label>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="Number">
                  <label for="icon_prefix3">Horas Extra</label>
                </div>
              </div>
            </form>
          </div>  
        </div>
        </div> 
        <div class="row">
          <form class="col s12">
            <div class="center">
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
            </div>
          </form>
        </div>
      </div>
    </div>
    <!-- Captura Domingo-->
    <div id="test-swipe-7" class="col s12">
      <div class="container">
          <h4 class="center">Domingo</h4>
          <p>Francisco perez vargas</p>
        <div class="row">
          <form class="col s6">
            <div class="row">
              <div class="input-field col s12">
                <i class="material-icons prefix">access_time</i>
                <input type="text" class="timepicker">
                <label for="icon_prefix3">Entrada</label>
              </div>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="text" class="timepicker">
                  <label for="icon_prefix4">Salida</label>
                </div>
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <form class="col s6">
            <div class="input-field col s12">
              <select>
                <option value="1">Asistencia</option>
                <option value="2">Inasistencia</option>
                <option value="3">Permiso</option>
                <option value="4">Vacaciones</option>
                <option value="5">Festivo</option>
              </select>
                <label>Asistencia</label>
            </div>
          </form>
          <div class="row">
            <form class="col s6">
              <div class="row">
                <div class="input-field col s12">
                  <i class="material-icons prefix">access_time</i>
                  <input type="Number">
                  <label for="icon_prefix3">Horas Extra</label>
                </div>
              </div>
            </form>
          </div>  
        </div>
        </div> 
        <div class="row">
          <form class="col s12">
            <div class="center">
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Cancelar</a>
              <a href="lista_viñedo.php" class="modal-close waves-effect green accent-4 btn">Aceptar</a>
            </div>
          </form>
        </div>
      </div>
    </div>
  </body>
</html>